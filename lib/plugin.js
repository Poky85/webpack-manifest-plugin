...
function ManifestPlugin(opts) {
  this.opts = _.assign({
    ...
    format: 'json',
    process: function(manifestJson) {
      return manifestJson;
    },
    postProcess: function(manifestString) {
...
ManifestPlugin.prototype.apply = function(compiler) {
  ...
  var process = this.opts.process;
  var postProcess = this.opts.postProcess;
  var moduleAssets = {};
  ...
  compiler.plugin('emit', function(compilation, compileCallback) {
    ...
    if (format.toLowerCase() === 'yml') {
      output = YAML.stringify(manifest, 4, 2);
    } else {
